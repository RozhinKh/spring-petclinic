...
/**
 * Parser for JFR (Java Flight Recorder) parsed metrics.
 * Extracts: GC pause times, thread counts, memory allocation, blocking event data
 */
public class JfrMetricsParser {

	private static final ObjectMapper mapper = new ObjectMapper();

	public List<NormalizedMetric> parse(File jfrMetricsFile, String variant, Integer runNumber)
			throws IOException {
	...
	private List<NormalizedMetric> parseGcMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (gcNode.isObject()) {
			...
			if (!Double.isNaN(gcPauseAvg)) {
				metrics.add(new NormalizedMetric("gc_pause_time_avg", gcPauseAvg, "ms", variant,
						runNumber, "gc", "jfr"));
			}

			Double gcPauseMax = gcNode.path("pause_time_max_ms").asDouble(Double.NaN);
			if (!Double.isNaN(gcPauseMax)) {
				metrics.add(new NormalizedMetric("gc_pause_time_max", gcPauseMax, "ms", variant,
						runNumber, "gc", "jfr"));
			}

			Double gcCount = gcNode.path("collection_count").asDouble(Double.NaN);
			if (!Double.isNaN(gcCount)) {
				metrics.add(new NormalizedMetric("gc_collection_count", gcCount, "count",
						variant, runNumber, "gc", "jfr"));
			}

			Double gcDuration = gcNode.path("total_pause_duration_ms").asDouble(Double.NaN);
			if (!Double.isNaN(gcDuration)) {
				metrics.add(new NormalizedMetric("gc_total_pause_duration", gcDuration, "ms",
						variant, runNumber, "gc", "jfr"));
			}
		}
	...
	private List<NormalizedMetric> parseThreadMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (threadNode.isObject()) {
			...
			if (!Double.isNaN(threadCount)) {
				metrics.add(new NormalizedMetric("thread_count", threadCount, "count", variant,
			...
			if (!Double.isNaN(peakThreadCount)) {
				metrics.add(new NormalizedMetric("thread_peak_count", peakThreadCount, "count",
						variant, runNumber, "threading", "jfr"));
			}

			Double virtualThreadCount = threadNode.path("virtual_thread_count")
					.asDouble(Double.NaN);
			if (!Double.isNaN(virtualThreadCount)) {
				metrics.add(new NormalizedMetric("virtual_thread_count", virtualThreadCount,
						"count", variant, runNumber, "threading", "jfr"));
			}
		}
	...
	private List<NormalizedMetric> parseMemoryMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (memoryNode.isObject()) {
			...
			if (!Double.isNaN(heapUsed)) {
				metrics.add(new NormalizedMetric("memory_heap_used", heapUsed, "MB", variant,
						runNumber, "memory", "jfr"));
			}

			Double heapMax = memoryNode.path("heap_max_mb").asDouble(Double.NaN);
			if (!Double.isNaN(heapMax)) {
				metrics.add(new NormalizedMetric("memory_heap_max", heapMax, "MB", variant,
						runNumber, "memory", "jfr"));
			}

			Double allocRate = memoryNode.path("allocation_rate_mb_per_sec")
					.asDouble(Double.NaN);
			if (!Double.isNaN(allocRate)) {
				metrics.add(new NormalizedMetric("memory_allocation_rate", allocRate, "MB/sec",
						variant, runNumber, "memory", "jfr"));
			}
		}
	...
	private List<NormalizedMetric> parseBlockingMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (blockingNode.isObject()) {
			...
			if (!Double.isNaN(blockingCount)) {
				metrics.add(new NormalizedMetric("blocking_event_count", blockingCount, "count",
						variant, runNumber, "blocking", "jfr"));
			}

			Double blockingDuration = blockingNode.path("total_duration_ms").asDouble(Double.NaN);
			if (!Double.isNaN(blockingDuration)) {
				metrics.add(new NormalizedMetric("blocking_total_duration", blockingDuration,
						"ms", variant, runNumber, "blocking", "jfr"));
			}
		}
