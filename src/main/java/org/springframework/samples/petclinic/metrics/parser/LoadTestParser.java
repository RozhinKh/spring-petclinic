...
public class LoadTestParser {
	...
	public List<NormalizedMetric> parse(File loadTestFile, String variant, Integer runNumber) throws IOException {
	...
	private List<NormalizedMetric> parseLatencyMetrics(JsonNode root, String variant, Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (latencyNode.isObject()) {
			...
			if (!Double.isNaN(p50)) {
				metrics.add(new NormalizedMetric("latency_p50", p50, "ms", variant, runNumber, "latency", "load_test"));
			}

			Double p95 = latencyNode.path("p95_ms").asDouble(Double.NaN);
			if (!Double.isNaN(p95)) {
				metrics.add(new NormalizedMetric("latency_p95", p95, "ms", variant, runNumber, "latency", "load_test"));
			}

			Double p99 = latencyNode.path("p99_ms").asDouble(Double.NaN);
			if (!Double.isNaN(p99)) {
				metrics.add(new NormalizedMetric("latency_p99", p99, "ms", variant, runNumber, "latency", "load_test"));
			}

			Double p999 = latencyNode.path("p99_9_ms").asDouble(Double.NaN);
			if (!Double.isNaN(p999)) {
				metrics
					.add(new NormalizedMetric("latency_p99_9", p999, "ms", variant, runNumber, "latency", "load_test"));
			}

			Double avg = latencyNode.path("avg_ms").asDouble(Double.NaN);
			if (!Double.isNaN(avg)) {
				metrics.add(new NormalizedMetric("latency_avg", avg, "ms", variant, runNumber, "latency", "load_test"));
			}

			Double min = latencyNode.path("min_ms").asDouble(Double.NaN);
			if (!Double.isNaN(min)) {
				metrics.add(new NormalizedMetric("latency_min", min, "ms", variant, runNumber, "latency", "load_test"));
			}

			Double max = latencyNode.path("max_ms").asDouble(Double.NaN);
			if (!Double.isNaN(max)) {
				metrics.add(new NormalizedMetric("latency_max", max, "ms", variant, runNumber, "latency", "load_test"));
			}
		}
	...
	private List<NormalizedMetric> parseThroughputMetrics(JsonNode root, String variant, Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (throughputNode.isObject()) {
			...
			if (!Double.isNaN(rps)) {
				metrics.add(new NormalizedMetric("throughput_rps", rps, "req/sec", variant, runNumber, "throughput",
						"load_test"));
			}

			Double totalRequests = throughputNode.path("total_requests").asDouble(Double.NaN);
			if (!Double.isNaN(totalRequests)) {
				metrics.add(new NormalizedMetric("load_test_total_requests", totalRequests, "count", variant, runNumber,
	...
	private List<NormalizedMetric> parseErrorMetrics(JsonNode root, String variant, Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (errorNode.isObject()) {
			...
			if (!Double.isNaN(errorCount)) {
				metrics.add(new NormalizedMetric("error_count", errorCount, "count", variant, runNumber, "throughput",
						"load_test"));
			}

			Double errorRate = errorNode.path("error_rate_percent").asDouble(Double.NaN);
			if (!Double.isNaN(errorRate)) {
				metrics.add(new NormalizedMetric("error_rate", errorRate, "%", variant, runNumber, "throughput",
						"load_test"));
			}
		}
