...
/**
 * Main orchestrator for result aggregation. Coordinates parsing of all data sources,
 * normalization, aggregation, and comparison table generation.
 */
public class ResultAggregator {
	 ...
	 */
	public AggregationResult aggregate(String baseDir, String variant, Integer runNumber) throws Exception {
	 ...
	 */
	public Map<String, List<AggregatedMetric>> aggregateMultipleRuns(String baseDir, String variant,
			List<Integer> runNumbers) throws Exception {
		List<NormalizedMetric> allMetrics = new ArrayList<>();
		...
		for (Integer runNumber : runNumbers) {
			try {
				...
				System.err.println("Warning: Failed to parse run " + runNumber + " for variant " + variant + ": "
						+ e.getMessage());
			}
		}
	 ...
	 */
	public Map<String, List<MetricComparison>> generateComparisons(List<AggregationResult> variantResults) {
	 ...
	 */
	public void exportResults(List<AggregatedMetric> aggregatedMetrics, String outputDir) throws IOException {
		ExportMetadata metadata = new ExportMetadata();
		exporter.export(aggregatedMetrics, outputDir, null, metadata);
	}

	// Parsing helper methods
	private List<NormalizedMetric> parseJmhResults(File baseDir, String variant, Integer runNumber) {
		try {
	...
	private List<NormalizedMetric> parseJfrResults(File baseDir, String variant, Integer runNumber) {
		try {
	...
	private List<NormalizedMetric> parseTestResults(File baseDir, String variant, Integer runNumber) {
	...
	private List<NormalizedMetric> parseLoadTestResults(File baseDir, String variant, Integer runNumber) {
		try {
	...
	private List<NormalizedMetric> parseActuatorResults(File baseDir, String variant, Integer runNumber) {
		try {
	...
	private List<NormalizedMetric> parseBlockingResults(File baseDir, String variant, Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		try {
	...
	private List<NormalizedMetric> parseModernizationResults(File baseDir, String variant, Integer runNumber) {
		try {
	...
	private File findFile(File directory, String namePattern, String extension) {
		...
		File[] files = directory.listFiles((dir, name) -> name.contains(namePattern) && name.endsWith(extension));
