...
/**
 * Benchmark metrics harness for integrating with JMH/JFR benchmark execution. Provides
 * methods to start/stop metrics collection during benchmark phases. Supports correlation
 * of metrics with JMH benchmark windows using matching timestamps.
 */
@Component
public class BenchmarkMetricsHarness {
	...
	public BenchmarkMetricsHarness(MetricsCollector metricsCollector,
			CaffeineCacheMetricsCollector cacheMetricsCollector, HttpMetricsListener httpMetricsListener) {
	...
	public void startBenchmark(String benchmarkName, String variant, int pollIntervalSeconds) {
		logger.info("Starting metrics collection for benchmark: {} (variant: {})", benchmarkName, variant);
	...
	public String stopBenchmark(String benchmarkName, String variant) {
		logger.info("Stopping metrics collection for benchmark: {} (variant: {})", benchmarkName, variant);
	...
	private void logCacheMetrics() {
		Map<String, CaffeineCacheMetricsCollector.CacheMetrics> allMetrics = cacheMetricsCollector.getAllMetrics();

		if (!allMetrics.isEmpty()) {
			logger.info("=== Cache Metrics Summary ===");
			for (Map.Entry<String, CaffeineCacheMetricsCollector.CacheMetrics> entry : allMetrics.entrySet()) {
				CaffeineCacheMetricsCollector.CacheMetrics metrics = entry.getValue();
				logger.info("Cache: {}", metrics.getCacheName());
				logger.info("  Hits: {}, Misses: {}, Hit Rate: {:.2f}%", metrics.getHits(), metrics.getMisses(),
						metrics.getHitRate());
				logger.info("  Evictions: {}, Removals: {}, Size: {}", metrics.getEvictions(), metrics.getRemovals(),
						metrics.getSize());
			}
		}
	}
