...
public class ActuatorMetricsParser {
	...
	public List<NormalizedMetric> parse(File actuatorMetricsFile, String variant,
	...
	private List<NormalizedMetric> parseHttpMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (httpNode.isObject()) {
			...
			if (!Double.isNaN(requestCount)) {
				metrics.add(new NormalizedMetric("http_request_count", requestCount, "count",
						variant, runNumber, "throughput", "actuator"));
			}

			Double avgResponse = httpNode.path("avg_response_time_ms").asDouble(Double.NaN);
			if (!Double.isNaN(avgResponse)) {
				metrics.add(new NormalizedMetric("http_avg_response_time", avgResponse, "ms",
						variant, runNumber, "latency", "actuator"));
			}

			Double maxResponse = httpNode.path("max_response_time_ms").asDouble(Double.NaN);
			if (!Double.isNaN(maxResponse)) {
				metrics.add(new NormalizedMetric("http_max_response_time", maxResponse, "ms",
						variant, runNumber, "latency", "actuator"));
			}
		}
	...
	private List<NormalizedMetric> parseJvmMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (jvmNode.isObject()) {
			...
			if (memoryNode.isObject()) {
				...
				if (!Double.isNaN(heapUsed)) {
					metrics.add(new NormalizedMetric("jvm_memory_heap_used", heapUsed, "MB",
							variant, runNumber, "memory", "actuator"));
				}

				Double heapMax = memoryNode.path("heap_max_mb").asDouble(Double.NaN);
				if (!Double.isNaN(heapMax)) {
					metrics.add(new NormalizedMetric("jvm_memory_heap_max", heapMax, "MB",
							variant, runNumber, "memory", "actuator"));
				}
			}
			...
			if (gcNode.isObject()) {
				...
				if (!Double.isNaN(gcTime)) {
					metrics.add(new NormalizedMetric("jvm_gc_time", gcTime, "ms", variant,
							runNumber, "gc", "actuator"));
				}

				Double gcCount = gcNode.path("gc_count").asDouble(Double.NaN);
				if (!Double.isNaN(gcCount)) {
					metrics.add(new NormalizedMetric("jvm_gc_count", gcCount, "count",
							variant, runNumber, "gc", "actuator"));
				}
			}
			...
			if (threadNode.isObject()) {
				...
				if (!Double.isNaN(threadCount)) {
					metrics.add(new NormalizedMetric("jvm_thread_count", threadCount, "count",
							variant, runNumber, "threading", "actuator"));
				}

				Double peakThreadCount = threadNode.path("peak_count").asDouble(Double.NaN);
				if (!Double.isNaN(peakThreadCount)) {
					metrics.add(new NormalizedMetric("jvm_thread_peak_count", peakThreadCount,
							"count", variant, runNumber, "threading", "actuator"));
				}
			}
		}
	...
	private List<NormalizedMetric> parseCacheMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (cacheNode.isObject()) {
			...
			if (!Double.isNaN(hitCount)) {
				metrics.add(new NormalizedMetric("cache_hit_count", hitCount, "count", variant,
						runNumber, "throughput", "actuator"));
			}

			Double missCount = cacheNode.path("miss_count").asDouble(Double.NaN);
			if (!Double.isNaN(missCount)) {
				metrics.add(new NormalizedMetric("cache_miss_count", missCount, "count",
						variant, runNumber, "throughput", "actuator"));
			}
			...
			if (!Double.isNaN(hitCount) && !Double.isNaN(missCount)) {
				...
				metrics.add(new NormalizedMetric("cache_hit_rate", hitRate, "%", variant,
						runNumber, "throughput", "actuator"));
			}
		}
	...
	private List<NormalizedMetric> parseDatabaseMetrics(JsonNode root, String variant,
			Integer runNumber) {
		List<NormalizedMetric> metrics = new ArrayList<>();
		...
		if (dbNode.isObject()) {
			...
			if (!Double.isNaN(activeConnections)) {
				metrics.add(new NormalizedMetric("db_active_connections", activeConnections,
						"count", variant, runNumber, "throughput", "actuator"));
			}

			Double poolSize = dbNode.path("pool_size").asDouble(Double.NaN);
			if (!Double.isNaN(poolSize)) {
				metrics.add(new NormalizedMetric("db_pool_size", poolSize, "count", variant,
						runNumber, "throughput", "actuator"));
			}

			Double queryTime = dbNode.path("avg_query_time_ms").asDouble(Double.NaN);
			if (!Double.isNaN(queryTime)) {
				metrics.add(new NormalizedMetric("db_avg_query_time", queryTime, "ms",
						variant, runNumber, "latency", "actuator"));
			}
		}
